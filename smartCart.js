// src/scripts/smartCart.js v2.1.6
...
;(() => {
  ...
  const SmartCart = {
    ...
    findActiveCampaignForProduct(productId) {
      ...
      const activeCampaign = this.campaigns.find((campaign) => {
        ...
        try {
          ...
          startTime = campaign.startTime?._seconds
        ...
        if (!(endTime instanceof Date && !isNaN(endTime)) || !(startTime instanceof Date && !isNaN(startTime))) {
          return false
        }

        const originalDuration = endTime - startTime
        this.originalDurations.set(campaign.id, originalDuration)

        return hasProduct && campaign.status === "active"
      })
    ...
    createCountdownTimer(campaign, productId) {
      ...
      // Retrieve stored end time from localStorage
      const storedEndTime = localStorage.getItem(`timer_${productId}`)
      if (storedEndTime) {
        endTime = new Date(Number.parseInt(storedEndTime))
      }
    ...
    createProductCardTimer(campaign, productId) {
      ...
      // Retrieve stored end time from localStorage
      const storedEndTime = localStorage.getItem(`timer_card_${productId}`)
      if (storedEndTime) {
        endTime = new Date(Number.parseInt(storedEndTime))
      }
    ...
    updateAllTimers() {
      ...
      this.activeTimers.forEach((timer, id) => {
        ...
        if (timeDiff <= 0 && timer.campaign?.timerSettings?.autoRestart && timer.originalDuration) {
          const restartCount = Math.floor(Math.abs(timeDiff) / timer.originalDuration) + 1
          const newEndTime = new Date(timer.endTime.getTime() + restartCount * timer.originalDuration)
          ...
          const storageKey = id.startsWith("card-") ? `timer_card_${id.replace("card-", "")}` : `timer_${id}`
          localStorage.setItem(storageKey, newEndTime.getTime().toString())
    ...
    setupProductCardTimers() {
      const productCardSelectors = [".product-item", ".card.card-product"]
      ...
      allProductCards.forEach((card) => {
        let productId = null
        const idSelectors = ["[data-wishlist-id]", 'input[name="product_id"]', "#product-id", ".js-add-to-cart"]
        ...
        for (const idSelector of idSelectors) {
          ...
          if (element) {
            productId =
              element.getAttribute("data-wishlist-id") ||
    ...
    initialize() {
      ...
      this.clearTimerStorage() // Add this line

      const isProductPage =
        document.querySelector(".product.products-details-page") ||
      ...
      if (isProductPage) {
        ...
        const productId = wishlistBtn?.getAttribute("data-wishlist-id") || productForm?.getAttribute("data-product-id")
