...
// Add these new helper functions inside the IIFE in smartCart.js, just before the SmartCart object:

const saveTimerState = (productId, endTime, originalDuration) => {
  const timerState = {
    endTime: endTime.getTime(),
    originalDuration: originalDuration,
    lastChecked: new Date().getTime()
  };
  localStorage.setItem(`smartCartTimer_${productId}`, JSON.stringify(timerState));
};

const loadTimerState = (productId) => {
  const savedState = localStorage.getItem(`smartCartTimer_${productId}`);
  ...
  const SmartCart = {
    ...
    findActiveCampaignForProduct(productId) {
      ...
      const activeCampaign = this.campaigns.find(campaign => {
        ...
        if (savedState && campaign.timerSettings?.autoRestart) {
          ...
          if (remainingTime <= 0) {
            // If timer had ended, calculate new end time based on original duration
            ...
            // If timer hadn't ended, adjust for passed time
        ...
        if (!this.originalDurations.has(campaign.id)) {
          ...
          this.originalDurations.set(campaign.id, endTime - startTime);
        }
    ...
    updateAllTimers() {
      ...
      this.activeTimers.forEach((timer, id) => {
        ...
        const originalDuration = timer.originalDuration || 
          (timer.campaign && this.originalDurations.get(timer.campaign.id));
    
        // Handle auto-restart
        if (timeDiff <= 0 && timer.campaign?.timerSettings?.autoRestart && originalDuration) {
          const cycles = Math.floor(Math.abs(timeDiff) / originalDuration);
          const newEndTime = new Date(now.getTime() + (originalDuration - 
            (Math.abs(timeDiff) - (cycles * originalDuration))));
          ...
          saveTimerState(productId, newEndTime, originalDuration);
        }
